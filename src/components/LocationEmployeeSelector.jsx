...
function LocationEmployeeSelector({ groupedData, employeeScans }) {
    ...
    const perHourAvg = currentEmployeeData
        ? calculatePerHourAverage(currentEmployeeData.firstTime, currentEmployeeData.lastTime, currentEmployeeData.orderIds.size)
        : '0.0';
    ...
    if (isReturnChecking) {
        const scansAscending = [...uniqueScans].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
        for (let i = 1; i < scansAscending.length; i++) {
            const diff = new Date(scansAscending[i].timestamp) - new Date(scansAscending[i - 1].timestamp);
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {uniqueScans?.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))?.map((scan, index) => {
                                        ...
                                        return (
                                            <tr
                                                key={index}
                                                className={`transition-colors duration-150 border-b border-gray-100 ${isRapidScan ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-gray-50'
                                                    }`}
                                            >
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className={`text-sm font-medium ${isRapidScan ? 'text-red-700' : 'text-gray-900'}`}>
                                                        {index + 1}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className={`text-sm font-mono font-semibold ${isRapidScan ? 'text-red-700' : 'text-gray-900'}`}>
                                                        {scan.orderId}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className={`text-sm ${isRapidScan ? 'text-red-600' : 'text-gray-600'}`}>
                                                        {scan.styleNumber}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className={`text-sm ${isRapidScan ? 'text-red-600 font-semibold' : 'text-gray-500'}`}>
                                                        {scan.scannedTime}
                                                    </div>
                                                </td>
                                            </tr>
